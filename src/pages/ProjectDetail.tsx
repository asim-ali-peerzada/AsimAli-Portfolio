...
// Helper component for strategic bolding
...
// Helper for dynamic project impact metrics
...
// Helper for formatted skimmable role lists
      ...
      return [
        {
          ...
          text: 'Designed relational database schema and state-machine logistics workflows for item-level tracking.',
        },
        {
          category: 'Backend & Automation',
          text: 'Built IMAP email ingestion engine, regex data extraction pipelines, and background queue workers.',
        },
        {
          category: 'Security & Identity',
          text: 'Integrated external Single Sign-On (SSO) authentication with granular role-based access control.',
        },
        {
          category: 'Frontend & Dashboards',
          text: 'Built administrative monitoring dashboards, Livewire UI, and real-time operational interfaces.',
        },
      ];
    case 'sales-&-contact-management-(ccms)':
      return [
        {
          category: 'Architecture',
          text: 'Designed complex relational schema for multi-level reporting hierarchies (CEO down to Sales Reps).',
        },
        {
          category: 'Backend & APIs',
          text: 'Architected RESTful API layer in Laravel 11 secured with Sanctum token authorization.',
        },
        {
          category: 'Workflow Automation',
          text: 'Implemented automated state-based lead assignment logic and quote-to-order pipeline.',
        },
        {
          category: 'Analytics & Dashboards',
          text: 'Built aggregation endpoints powering real-time executive sales dashboards.',
        },
      ];
    case 'enterprise-single-sign-on':
      return [
        {
          ...
          text: 'Designed JWT authentication flows, Google 2FA protocols, and cross-domain token blacklisting.',
        },
        {
          category: 'Backend Integrations',
          text: 'Developed external API synchronization logic and queued background job processing.',
        },
        {
          category: 'Data Pipelines',
          text: 'Built queue-based engine for bulk Excel infrastructure data processing and import.',
        },
        {
          category: 'Frontend & Governance',
          text: 'Delivered React-based control panel utilizing Recharts for real-time access monitoring.',
        },
      ];
    case 'ai-assistant-platform':
      return [
        {
          ...
          text: 'Decoupled system into Laravel 12 API foundation, FastAPI AI intelligence engine, and React 19 UI.',
        },
        {
          category: 'AI & RAG Engineering',
          text: 'Built Retrieval-Augmented Generation pipeline with Pinecone vector DB and LLM integrations.',
        },
        {
          category: 'Multi-Tenant Security',
          text: 'Engineered strict workspace tenant data isolation, RBAC, subscription billing, and audit logs.',
        },
        {
          category: 'Multimodal Workflows',
          text: 'Integrated Whisper/Deepgram STT and ElevenLabs/OpenAI TTS for voice automation.',
        },
      ];
    case 'zametrix':
      return [
        {
          ...
          text: 'Designed Laravel REST APIs and responsive React/TypeScript frontend.',
        },
        {
          category: 'Governance & RBAC',
          text: 'Architected 4-role access hierarchy (Admin, Partner, Agent, Public) with Spatie RBAC.',
        },
        {
          category: 'Data Pipelines',
          text: 'Engineered structured reporting and administrative verification pipeline for market updates.',
        },
        {
          category: 'Analytics & CRM',
          text: 'Built property pulse comparison tools, PDF generation, and lead routing CRM module.',
        },
      ];
    case 'genealogy-saas-platform':
      return [
        {
          ...
          text: 'Designed multi-tenant data isolation model and GEDCOM archival schemas.',
        },
        {
          category: 'API Integrations',
          text: 'Integrated NARA and OpenArch external historical archive APIs.',
        },
        {
        ...
        : [
            {
              ...
              text: 'Designed and built full application workflow.',
            },
          ];
  }
};
...
const ProjectDetail = () => {
  ...
  // Lookup the project by slug if location state is empty
  const slug = decodeURIComponent(params.slug || '');
  const project = projects.find((p) => p.slug === slug) || location.state?.project;
  ...
  if (!project) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-bg font-sans">
        <div className="max-w-md p-8 border border-brand-border bg-white rounded-[24px] text-center space-y-4">
  ...
  const problemTexts: Record<string, string> = {
    ...
    'sales-&-contact-management-(ccms)':
      'The organization was struggling with **fragmented client data**, untracked sales pipelines, and manual lead distribution. They required a centralized system that could **enforce strict data visibility** based on corporate hierarchy and geographic territories while providing real-time executive sales analytics.',
    ...
    'ai-assistant-platform':
      'Businesses increasingly require AI automation but struggle to **securely integrate proprietary data**. Implementing intelligent chatbots often leads to **disconnected workflows and poor data isolation**, lacking backend infrastructure to manage multi-tenant access, document processing, and multi-channel customer conversations.',
    zametrix:
      'The real estate industry suffers from **fragmented, inconsistent, and unstructured property data**. Investors lack reliable location-based intelligence, field agents rely on manual reporting, and administrative teams struggle with **weak visibility and inefficient moderation workflows**.',
  };

  const solutionTexts: Record<string, string> = {
    'shipment-tracker-&-ims':
      'Architected an **automated data pipeline that connects directly to an IMAP mailbox** to fetch, parse, and classify incoming email notifications into structured shipment records. The system utilizes background queue processing for **reliable data extraction** and enforces a multi-step business workflow, allowing administrative teams to **track item-level quantities, flag missing equipment, and monitor technician transfers** through a centralized, SSO-secured dashboard.',
    'sales-&-contact-management-(ccms)':
    ...
    zametrix:
      'Architected a **centralized, multi-role intelligence platform** that standardizes property data collection and moderation. Field agent submissions are **routed through administrative verification** before feeding a public analytics portal where users can **compare locations and explore property trends**.',
  };
    ...
    <div className="min-h-screen bg-white text-[#0a0a0a] font-['Urbanist',sans-serif] antialiased relative">
      ...
      <div className="pt-28 md:pt-36 pb-24 px-4 md:px-8 max-w-[1600px] mx-auto space-y-12 md:space-y-16">
        ...
        <div className="w-full bg-slate-50/50 border border-slate-100/80 rounded-[40px] pt-10 px-4 md:px-8 pb-8 flex flex-col items-center">
          ...
          {/* Main Screenshot with Diffuse Shadow */}
          ...
          {/* ITEM 1: REFINED LIGHT & CLEAN TL;DR IMPACT BAR */}
          ...
          {/* METADATA CARDS (SERVICES & TECH STACK) */}
            ...
            <div className="lg:col-span-6 bg-white border border-gray-200/60 rounded-[32px] p-6 lg:p-8 space-y-5 h-full shadow-xs">
              ...
              <div className="flex flex-wrap gap-2 lg:gap-3">
                {project.services?.map((s, idx) => (
                  <span
                    key={idx}
                    className="px-4 py-2 rounded-full bg-[#f1f5f9] text-[#0a0a0a] text-[13px] lg:text-[14px] font-medium tracking-wide font-['Urbanist',sans-serif]"
                  >
                    {s}
                  </span>
                ))}
        ...
        {/* OVERVIEW & PROBLEM CARDS GRID (WITH STRATEGIC BOLDING) */}
        <section className="flex flex-col lg:flex-row gap-6 w-full">
          ...
          {/* Problem Card with Strategic Bolding */}
        ...
        {/* ITEM 4 & 5: LIGHTENED SCREENSHOTS CAROUSEL & VISUAL AFFORDANCE CONTROLS */}
        <section className="w-full rounded-[40px] py-8 md:py-12 px-4 md:px-8 flex flex-col items-center bg-slate-50/40 border border-slate-100/60">
          <div className="w-full max-w-5xl flex flex-col items-center">
            ...
            {/* ITEM 5: VISUAL AFFORDANCE CONTROL DOCK */}
            <div className="flex items-center justify-center gap-4 mt-8 p-1.5 rounded-full bg-white border border-gray-200/80 shadow-md">
              ...
              <div className="px-4 py-1 rounded-full bg-[#f1f5f9] text-[13px] font-semibold text-[#0a0a0a] font-['Urbanist',sans-serif] tracking-wider">
                {activeSlide + 1} <span className="text-gray-400 font-normal">/</span>{' '}
        ...
        {/* SOLUTION, ROLE & HIGHLIGHTS CONTAINER */}
        <section className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
          {/* Left Column (Solution & Skimmable Role List) */}
          <div className="lg:w-[55%] space-y-12">
            {/* Solution with Strategic Bolding */}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {roleList.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-5 rounded-[20px] bg-[#f8fafc] border border-gray-200/60 flex items-start gap-4 transition-all hover:border-gray-300"
                  >
